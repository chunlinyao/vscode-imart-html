...
export interface HTMLDocumentRegions {
	getEmbeddedDocument(languageId: string, ignoreAttributeValues?: boolean): HTMLDocument;
	getLanguageRanges(range: Range): LanguageRange[];
	getLanguageAtPosition(position: Position): string | undefined;
	getLanguagesInDocument(): string[];
	getImportedScripts(): string[];
}
export interface HTMLDocument extends TextDocument {
...
export function getDocumentRegions(languageService: LanguageService, document: TextDocument): HTMLDocumentRegions {
	...
	return {
		...
		getEmbeddedDocument: (languageId: string, ignoreAttributeValues: boolean) => getEmbeddedDocument(document, regions, languageId, ignoreAttributeValues, importedScripts),
...
function getEmbeddedDocument(document: TextDocument, contents: EmbeddedRegion[], languageId: string, ignoreAttributeValues: boolean, importedScripts: string[] ): HTMLDocument {
	let currentPos = 0;
	...
	const doc = TextDocument.create(document.uri, languageId, document.version, result) as HTMLDocument;
	doc.importedScripts = importedScripts;
	return doc;
}
